...
class InputManager:
    ...
    def get_events(self):
        ...
        for button in self.buttons:
            ...
            # determine what something like "Y" actually means in terms of the joysticl
            config = self.joystick_config(button)
            if config != None:

                # if the button is configured to an actual button...
                if config[0] == 'is_button':
                    pushed = self.joystick.get_button(config[1])
                    if pushed != self.joystick.get_button(config[1]):
...
def main():
    ...
    while not input_manager.quit_attempt:
        ...
        # In the first phase, the user will be propted to configure the joystick by pressing
        # the ky that is indicated on the screen
...
# There was probably a more robust way of doing this with command line interaction
def wait_for_enter():
    try: input()
    except: pass
